...
// Expandable bio text
function ExpandableBio({ text, toggleText }: { text: string; toggleText: { more: string; less: string } }) {
  ...
  return (
    <div className="mb-3">
      ...
      {isLong && (
        ...
        >
          {expanded ? toggleText.less : toggleText.more}
...
// Labels for level display are now fetched via translations
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-32 pb-16">
      <div className="container mx-auto px-4 md:px-6">
        <div className="mb-12">
          <BackLink href="/#equipe" label={tNav('backHome')} />
          <h1 className="text-3xl md:text-5xl font-black tracking-tighter text-foreground mb-4">{t('title')}</h1>
        ...
        <p className="text-sm text-muted-foreground mb-6">
          {filteredMembers.length === 1 ? t('memberFound') : t('membersFound', { count: filteredMembers.length })}
        </p>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[13px] text-primary font-medium mb-2">
                        {member.role || t('levels.coordinator')}
                      </p>
                      <div className="w-full h-px bg-slate-200 dark:bg-slate-700 mb-2" />
                      {(member.bio || member.focus) && (
                        <ExpandableBio text={member.bio || member.focus || ''} toggleText={{ more: t('seeMore'), less: t('seeLess') }} />
                      )}
                ...
                {filteredMembers.filter(m => m.level !== 'coordinator').map((member, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[13px] text-primary font-medium mb-2">
                        {t(`levels.${member.level || 'researcher'}`)}
                      </p>
                      <div className="w-full h-px bg-slate-200 dark:bg-slate-700 mb-2" />
                      {member.bio && (
                        <ExpandableBio text={member.bio} toggleText={{ more: t('seeMore'), less: t('seeLess') }} />
                      )}
                      <div className="flex items-center gap-2">
                        {member.lattes && member.lattes !== '#' && (
                          <Link href={member.lattes} target="_blank" title="Currículo Lattes" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-primary hover:text-white text-xs font-medium transition-colors">
