...
function ExpandableAbstract({ text, toggleText }: { text: string; toggleText: { more: string; less: string } }) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-32 pb-16">
      <div className="container mx-auto px-4 md:px-6">
        <div className="mb-12">
          <BackLink href="/#publicacoes" label={tNav('backHome')} />
          <div className="mb-8">
              ...
              <>
                {availableTypes.length > 1 && (
                  ...
                  >
                    ...
                    {availableTypes.map(tType => (
                      <option key={tType} value={tType}>{t(`types.${tType}`)}</option>
                    ))}
                  </select>
                )}
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          <span>•</span>
                          <span className="text-blue-600 dark:text-blue-400 font-bold">{t(`types.${pub.type}`)}</span>
                        </div>

                        {pub.abstract && <ExpandableAbstract text={pub.abstract} toggleText={{ more: t('viewMore'), less: t('viewLess') }} />}
